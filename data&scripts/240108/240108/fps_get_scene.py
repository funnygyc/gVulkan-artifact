...
result_df = pd.DataFrame(columns=['scene', 'method', 'gpunum', 'total_latency','lp','rw','ic','sb','pr'])
...
for scene in ['scene1', 'scene3', 'scene4', 'scene5']:
    for method in ['benchmark', 'multithread', 'gvulkan']:
        for gpunum in ['1', '2', '4']:
            ...
            server_log_file = os.path.join(folder_path, f'{gpunum}gpu/{gpunum}GPU_{scene}{method}720p32_server.log')
            if method == 'gvulkan':
...
# 将结果保存为 csv 文件
result_df.to_csv('gVulkan_scene_fps.csv', index=False)