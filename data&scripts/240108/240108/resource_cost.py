...
def extract_cpu_resource(log_file):
    time_diffs_cpu_resource = []
    times = 0
    with open(log_file, 'r') as f:
        for line in f:
            # resource_match = re.match(r'(\d+\.\d+)', line)
            # if resource_match:
            #     time_diffs_cpu_resource = float(resource_match.group(1))
            time_diffs_cpu_resource.append(float(line))
            times += 1                
    print(times)
    mean_time_diff_cpu_resource = np.mean(time_diffs_cpu_resource[10:30])
    return mean_time_diff_cpu_resource

def extract_gpu_resource(log_file):
    time_diffs_gpu_resource = []
    times = 0
    with open(log_file, 'r') as f:
        for line in f:
            # resource_match = re.match(r'(\d+\.\d+)', line)
            ...
            if match:
                time_diffs_gpu_resource.append(float(match.group(1)))
                times += 1                
    print(times)
    # print(time_diffs_gpu_resource)
    mean_time_diff_cpu_resource = np.mean(time_diffs_gpu_resource[40:120])
    return mean_time_diff_cpu_resource

def extract_gpu_mem_resource(log_file,gpunum):
    time_diffs_gpu_mem_resource = []
    times = 0
    with open(log_file, 'r') as f:
        for line in f:
            if './DemoServer' in line:
                line_parts = line.split('./DemoServer')
                match = re.search(r'\d+', line_parts[1].strip())
                if match:
                    time_diffs_gpu_mem_resource.append(float(match.group(0)))
                    times += 1                
    print(times)
    # print(time_diffs_gpu_mem_resource)
    if gpunum == '1':
        mean_time_diff_cpu_resource = np.mean(time_diffs_gpu_mem_resource[10:30])
    elif gpunum == '2':
        mean_time_diff_cpu_resource = np.mean(time_diffs_gpu_mem_resource[20:60])
    elif gpunum == '4':
        mean_time_diff_cpu_resource = np.mean(time_diffs_gpu_mem_resource[40:120])
    return mean_time_diff_cpu_resource


# 定义一个空 DataFrame 用于存储结果
result_df = pd.DataFrame(columns=['scene', 'method', 'gpunum', 'cpu','mem','gpu','gpu_mem'])


for scene in ['scene1', 'scene3', 'scene4', 'scene5']:
    for method in ['benchmark', 'multithread', 'gvulkan']:
        for gpunum in ['1', '2', '4']:
            ...
            cpu_log_file = os.path.join(folder_path, f'{gpunum}gpu/{gpunum}GPU_{scene}{method}720p32_cpu_utilization.txt')
            mem_log_file = os.path.join(folder_path, f'{gpunum}gpu/{gpunum}GPU_{scene}{method}720p32_memory_utilization.txt')
            gpu_log_file = os.path.join(folder_path, f'{gpunum}gpu/{gpunum}GPU_{scene}{method}720p32_gpu_utilization.txt')
            gpu_mem_log_file = os.path.join(folder_path, f'{gpunum}gpu/{gpunum}GPU_{scene}{method}720p32_gpu_memory_usage.txt')

            cpu  = extract_cpu_resource(cpu_log_file)
            mem  = extract_cpu_resource(mem_log_file)
            gpu  = extract_gpu_resource(gpu_log_file)
            gpu_mem  = extract_gpu_mem_resource(gpu_mem_log_file,gpunum)
            time_diff_df = pd.DataFrame({'scene': scene,
                                            'method': method,
                                            'gpunum': gpunum,
                                            'cpu': cpu,
                                            'mem': mem,
                                            'gpu': gpu,
                                            'gpu_mem': gpu_mem,},  index=[0])
...
# 将结果保存为 csv 文件
result_df.to_csv('gVulkan_scene_resource_cost.csv', index=False)