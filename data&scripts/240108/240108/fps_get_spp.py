...
result_df = pd.DataFrame(columns=['spp', 'method', 'gpunum', 'fps'])


for spp in ['4', '8', '16', '32', '64', '128', '256']:
    for method in ['benchmark', 'multithread', 'gvulkan']:
        for gpunum in ['1', '2', '4']:
            ...
            if spp == '32':
                ...
                folder_path = f'./spp/'
            server_log_file = os.path.join(folder_path, f'{gpunum}gpu/{gpunum}GPU_scene1{method}720p{spp}_server.log')
            server  = extract_server_latency(server_log_file)
            time_diff_df = pd.DataFrame({'spp': spp,
                                            'method': method,
                                            'gpunum': gpunum,
...
# 将结果保存为 csv 文件
result_df.to_csv('gVulkan_spp_fps.csv', index=False)